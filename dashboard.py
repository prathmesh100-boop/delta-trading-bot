...
New in V8:
  - Per-setup profitability panel (trend_pullback vs range_mean_rev vs structure_break)
  - Entry grade breakdown (A/B/C/D wins and losses)
  - Trade table now shows setup_type and entry_grade columns
  - Reads from trade_history.csv (enriched V8 format)
  - Equity curve now shows per-symbol lines if multiple symbols detected
"""
import asyncio
import json
import logging
...
def root(_ok: bool = Depends(_require_token)):
    return JSONResponse({"status": "ok", "version": "V8",
                         "endpoints": ["/dashboard", "/api/stats", "/api/trades", "/api/equity", "/stream"]})
...
<style>
  :root {
    ...
    --yellow: #fbbf24; --orange: #f97316; --purple: #a78bfa;
  }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .section h2 { font-size: 11px; font-weight: 600; color: var(--muted); margin-bottom: 8px;
  ...
  .analytics-panel h3 { font-size: 11px; color: var(--muted); text-transform: uppercase;
    letter-spacing: .5px; margin-bottom: 8px; }
  .analytics-row { display: flex; justify-content: space-between; align-items: center;
    padding: 5px 0; border-bottom: 1px solid rgba(45,55,72,.5); font-size: 12px; }
  ...
  thead th { padding: 7px 8px; text-align: left; color: var(--muted); font-size: 10px;
...
function updateStats(s) {
  ...
  if (Object.keys(ss).length === 0) {
    ...
    ssEl.innerHTML = Object.entries(ss).map(([setup, d]) => {
      ...
      const setupLabel = setup.replace('_', ' ');
      return `<div class="analytics-row">
        <span style="color:var(--text)">${setupLabel}</span>
        <span style="color:var(--muted);font-size:11px">${d.trades}T ${d.win_rate}%wr</span>
        <span class="${pnlClass}">${d.total_pnl >= 0 ? '+' : ''}${d.total_pnl.toFixed(3)}</span>
      </div>`;
    }).join('');
  }
  ...
  if (Object.keys(gs).length === 0) {
    ...
    gsEl.innerHTML = sorted.map(([grade, d]) => {
      ...
      return `<div class="analytics-row">
        ...
        <span style="color:var(--muted);font-size:11px">${d.trades} trades  ${d.win_rate}% wr</span>
        <span class="${pnlClass}">${d.total_pnl >= 0 ? '+' : ''}${d.total_pnl.toFixed(3)}</span>
      </div>`;
    }).join('');
  }
...
function updateTrades(trades) {
  ...
  const rows = [...trades].reverse().map(t => {
    ...
    const setup = (t.setup_type || '').replace('_', ' ');
    return `<tr>
      <td style="color:var(--muted)">${exitTime}</td>
      <td><b>${t.symbol || ''}</b></td>
      <td>${sideTag}</td>
      <td style="color:var(--muted);font-size:11px">${setup}</td>
